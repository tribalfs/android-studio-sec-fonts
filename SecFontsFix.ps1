...
function Run-SecFontsFix
{
    ...
    if (-not $found)
    {
        Write-Host "No plugins\design-tools\resources\layoutlib\data\fonts directory found in any subfolder of ProgramFiles\Android." -ForegroundColor Red
    }
    else
    {
        Write-Host "Done!"
    ...
    foreach ($installDir in $installDirs)
    {
        if (Test-Path $installDir)
        {
            ...
            Get-ChildItem -Directory | ForEach-Object {
                ...
                {
                    if (Test-Path "$dir\plugins\design-tools\resources\layoutlib\data\fonts")
                    {
                        Write-Host "Fonts_backup folder not found for $dir!"
                    }
                }
            }
        }
